...
var SqueezeBox = {
	...
	open: function(subject, options) {
		...
		var need_folder = true;
		var href_array = null;
		if (this.element.get('href') != null && this.element.get('href').indexOf('?') > 0) {
			href_array = this.element.get('href').split('?')[1].split('&');
		} else if (this.element.get('href') != null) {
			href_array = this.element.get('href').split('&');
		}
		
		if (href_array != null && href_array.length > 0) {
			var find_option = false;
			for (var hi = 0; hi < href_array.length; hi++) {
				var param = href_array[hi].split('=');
				if (param[0] == 'option') {
					find_option = true;
				}
				if (param[0] == 'option' && param[1] != 'com_media') {
					need_folder = false;
					break;
				}
			}
			if (find_option == false && need_folder == true) {
				need_folder = false;
			}
		}
		
		var tempFolder;
		if (document.id('tempFolder') != undefined) {
			tempFolder = document.id('tempFolder').value;
		} else if (href_array != null && need_folder == true && document.id('tempFolder') == undefined &&　(this.element.get('href').indexOf('&tofolder') < 0 || this.element.get('href').indexOf('&lockfolder') < 0)) {
			alert('对不起，您没有定义上传文件夹的路径！');
			return true;
		}
		this.url = ((this.element) ? (this.element.get('href')+(tempFolder ? '&tofolder='+tempFolder+'&lockfoldere='+tempFolder : '')) : subject) || this.options.url || '';
