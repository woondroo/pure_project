...
/**
 * Build the route for the com_integral component
 *
 * @param	array	An array of URL arguments
 *
 * @return	array	The URL arguments to use to assemble the subsequent URL.
 */
function IntegralBuildRoute(&$query)
{
	$segments = array();
	...
	$menu		= $app->getMenu();
	$params	= $app->getParams('com_integral');
	...
	if (isset($view) and ($view == 'integrals' or $view == 'integral' )) {
		if (isset($query['id'])) {
			$catid = $query['id'];
		} else {
			$catid = $mId;
 ...
 */
function IntegralParseRoute($segments)
{
	$vars = array();
	...
	if(is_numeric($seg_last_num) && count($segments) >= 2 && $segments[count($segments)-2]) {
		...
		$cateid = MulanDBUtil::getObjectBySql('SELECT id FROM #__categories WHERE extension="com_integral" and alias ="'.$segmentStr.'" ')->id;
		if ($cateid) {
			$vars['id'] = $cateid;
		}
		...
		if ($vars['id']) {
			...
			$obj = MulanDBUtil::getObjectBySql('SELECT id,catid FROM #__integral WHERE id ="'.$seg_last_num.'" ');
	...
	if ($is_tolist == true) {
		...
		$cateid = MulanDBUtil::getObjectBySql('SELECT id FROM #__categories WHERE extension="com_integral" and alias ="'.$segmentsStr_alias.'" ')->id;
		$vars['view'] = 'integrals';
		$vars['id']	= $cateid;
