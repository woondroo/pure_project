...
$itemLink = 'index.php?option=com_integral&view=integral&Itemid='.$itemid.($start > 0 ? '&start='.$start : '&limitstart=0').'&id='.$id;
...
<div class='integral-list list-area-n<?php echo $num_aline; ?> list-<?php echo $listway; ?>'>
	...
	<?php echo $item_elements == 'tdt' ? '<a'.($openway ? '' : ' target="_blank"').' href="'.$itemLink.'&pid='.$o->id.'&alias='.$o->alias.'"' : '<div'; ?><?php echo $key % $num_aline == 0 && $num_aline > 1 ? ' style="margin-left:0;"' : ''; ?> class='list-item list-item-n<?php echo $num_aline; ?> list-item-n<?php echo $num_aline; ?>-e<?php echo $item_elements; ?>'>
		...
		if ($item_elements == 'tb') {
			...
			$item_str .= ' class="list-bt" href="'.$itemLink.'&pid='.$o->id.'&alias='.$o->alias.'">'.($showbttext ? $bttext : '').$o->integral.'</a>';
		}
