...
/**
 * Build the route for the com_download component
 *
...
function DownloadBuildRoute(&$query)
{
	...
	if (isset($view) and ($view == 'downloads' or $view == 'download' )) {
		if (isset($query['id'])) {
			$catid = $query['id'];
		} else {
			$catid = $mId;
...
function DownloadParseRoute($segments)
{
	...
	if(is_numeric($seg_last_num) && count($segments) >= 2 && $segments[count($segments)-2]) {
		...
		$cateid = MulanDBUtil::getObjectBySql('SELECT id FROM #__categories WHERE extension="com_download" and alias ="'.$segmentStr.'" ')->id;
		if ($cateid) {
			$vars['id'] = $cateid;
		}
		...
		if ($vars['id']) {
			...
			$obj = MulanDBUtil::getObjectBySql('SELECT id,catid FROM #__download WHERE id ="'.$seg_last_num.'" ');
	...
	if ($is_tolist == true) {
		...
		$cateid = MulanDBUtil::getObjectBySql('SELECT id FROM #__categories WHERE extension="com_download" and alias ="'.$segmentsStr_alias.'" ')->id;
		$vars['view'] = 'downloads';
		$vars['id']	= $cateid;
