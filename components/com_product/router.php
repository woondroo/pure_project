...
/**
 * Build the route for the com_product component
 *
...
function ProductBuildRoute(&$query)
{
	...
	$params		= $app->getParams('com_product');
	...
	if (isset($view) and ($view == 'products' or $view == 'product' )) {
		if (isset($query['id'])) {
			$catid = $query['id'];
		} else {
			$catid = $mId;
...
function ProductParseRoute($segments)
{
	...
	if(is_numeric($seg_last_num) && count($segments) >= 2 && $segments[count($segments)-2]) {
		...
		$cateid = MulanDBUtil::getObjectBySql('SELECT id FROM #__categories WHERE extension="com_product" and alias ="'.$segmentStr.'" ')->id;
		if ($cateid) {
			$vars['id'] = $cateid;
		}
		...
		if ($vars['id']) {
			...
			$obj = MulanDBUtil::getObjectBySql('SELECT id,catid FROM #__product WHERE id ="'.$seg_last_num.'" ');
	...
	if ($is_tolist == true) {
		...
		$cateid = MulanDBUtil::getObjectBySql('SELECT id FROM #__categories WHERE extension="com_product" and alias ="'.$segmentsStr_alias.'" ')->id;
		$vars['view'] = 'products';
		$vars['id']	= $cateid;
