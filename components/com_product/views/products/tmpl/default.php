...
$imgcirclebg_pos = MulanToolsUtil::getMaterialPos($imgcirclebg);
$imgcirclebg = $imgcirclebg_pos['img'];
...
$titlepre_pos = MulanToolsUtil::getMaterialPos($titlepre);
$titlepre = $titlepre_pos['img'];
...
$insert_style = '';
if ($imgcirclebg) {
	$insert_style = '.list-img-area{background:url('.$base.$imgcirclebg.') '.(-1*$imgcirclebg_pos['x']).'px '.(-1*$imgcirclebg_pos['x']).'px}';
}

$product_item_width = max(intval($item_img_width)+intval($item_img_circle)*2,
						intval($item_title_width == '100%' ? 0 : $item_title_width),
						intval($item_bt_width == '100%' ? 0 : $item_bt_width));
if ($item_width != 'auto') $product_item_width = intval($item_width);
$products_items_width = $product_item_width*$num_aline + intval($item_pos[3])*($num_aline-1);
$insert_style .= '.product-list{width:'.$products_items_width.'px}.list-item{width:'.$product_item_width.'px;height:'.$item_height.';margin:'.implode(' ',$item_pos).';}';
?>
<div class="product-list list-<?php echo $listway; ?>">
	<?php
	if (count($this->items)) {
		foreach($this->items as $key=>$o) {
			...
			if ($num_aline == 1) {
				...
				$item_str .= $openway && $titlelink ? '' : ' target="_blank"';
				$item_str .= $titlelink ? ' href="'.$itemLink.'&pid='.$o->id.'&alias='.$o->alias.'"' : '';
				$item_str .= ' class="list-c-title"'.($item_elements == 'tt' ? ' style="background-image:url('.$base.$titlepre.') '.(-1*$titlepre_pos['x']).'px '.(-1*$titlepre_pos['y']).'px;"' : '').' title="'.$title.'">'.$sub_title;
				$item_str .= $titlelink ? '</a>' : '</div>';
