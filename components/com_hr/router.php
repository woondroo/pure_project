...
/**
 * Build the route for the com_hr component
 *
 * @param	array	An array of URL arguments
 *
 * @return	array	The URL arguments to use to assemble the subsequent URL.
 */
function HrBuildRoute(&$query)
{
	$segments = array();
	...
	if (isset($view) and ($view == 'hrs' or $view == 'hr' )) {
		if (isset($query['id'])) {
			$catid = $query['id'];
		} else {
			$catid = $mId;
		...
		$categories = JCategories::getInstance('Hr');
		$hrs = $categories->get($catid);
		if($hrs){
			$path = $hrs->getPath();
 ...
 */
function HrParseRoute($segments)
{
	$vars = array();
	...
	if(is_numeric($seg_last_num) && count($segments) >= 2 && $segments[count($segments)-2]) {
		...
		$cateid = MulanDBUtil::getObjectBySql('SELECT id FROM #__categories WHERE extension="com_hr" and alias ="'.$segmentStr.'" ')->id;
		if ($cateid) {
			$vars['id'] = $cateid;
		}
		...
		if ($vars['id']) {
			...
			$obj = MulanDBUtil::getObjectBySql('SELECT id,catid FROM #__hr WHERE id ="'.$seg_last_num.'" ');
	...
	if ($is_tolist == true) {
		...
		$cateid = MulanDBUtil::getObjectBySql('SELECT id FROM #__categories WHERE extension="com_hr" and alias ="'.$segmentsStr_alias.'" ')->id;
		$vars['view'] = 'hrs';
		$vars['id']	= $cateid;
