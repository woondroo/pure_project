...
class JFormFieldMedia extends JFormField
{
	...
	protected function getInput()
	{
		...
		/**
		 * 2011-12-15 wengebin Add
		 * 该段代码用于拼凑锁定文件夹和上传文件夹的代码。
		 */
		$upload_root_uri = 'img/';
		$upload_parent_uri = str_replace('com_','',JRequest::getVar('option'));
		$id = JRequest::getVar('id');
		$upload_uri .= $id
				? $upload_root_uri.$upload_parent_uri.'/'.$upload_parent_uri.'-'.$id
				: '';
		
		$html[] = '<div class="button2-left">';
		$html[] = '	<div class="blank">';
		$html[] = '		<a class="modal" title="'.JText::_('JLIB_FORM_BUTTON_SELECT').'"' .
					' href="'.($this->element['readonly'] ? '' : ($link ? $link : 'index.php?option=com_media&amp;view=images&amp;tmpl=component&amp;asset='.$asset.'&amp;author='.$this->form->getValue($authorField)) . '&amp;fieldid='.$this->id.'&amp;folder='.$folder. ($id ? '&amp;tofolder='.$upload_uri.'&amp;lockfolder='.$upload_uri : '')).'"' .
