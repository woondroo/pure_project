...
//		$db = JFactory::getDbo();
	 ...
	 * 新增自定义验证方法，自定义组件都需要用此验证方法进行验证，为避开自定义组件无 lft 和 rgt 的情况。
	 */
	public function authoriseInCustom($action, $assetname = null)
	{
		// Make sure we only check for core.admin once during the run.
		if ($this->isRoot === null) {
			$this->isRoot = false;
		...
		return $this->isRoot ? true : JAccess::checkInCustom($this->id, $action, $assetname);
	}
	
	/**
	 * 2011-12-6 wengebin edit
	 * 新增权限验证方法，根据组ID验证该组是否有访问权限。
	 */
	public function authoriseByGroupId($groupId, $action, $assetname = null)
	{
		return JAccess::checkGroupByGroupId($groupId, $action, $assetname);
	}
}
