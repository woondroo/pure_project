...
abstract class JFactory
{
	 ...
	 * @see    JDatabase
	 * 
	 * 2012-01-08 wengebin edit
	 * Add param $conf!
	 */
	public static function getDbo($conf=null)
	{
		$sign = $conf != null && is_array($conf) ? $conf['sign'] : 'nosign';
		/**
		 * 2012-06-28 wengebin edit
		 * 
		 * 添加 conf 参数用来使用自定义数据库连接！
		 */
		if ($conf != null && is_array($conf) && !self::$database[$sign]) {
			$conf_get = self::getConfig();
			$debug	= $conf['debug'];
			
			$conf_get->set('host',$conf['host']);
			$conf_get->set('user',$conf['user']);
			$conf_get->set('password',$conf['password']);
			$conf_get->set('db',$conf['db']);
			$conf_get->set('dbprefix',$conf['dbprefix']);
			$conf_get->set('dbtype',$conf['dbtype']);
			$conf_get->set('debug',$conf['debug']);
			
			self::$config = $conf_get;
			self::$database[$sign] = self::_createDbo();
			self::$database[$sign]->debug($debug);
		} else if (!self::$database[$sign]) {
