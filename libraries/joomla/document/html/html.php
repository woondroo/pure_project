...
class JDocumentHTML extends JDocument
{
		...
		if (isset($data['style'])) {
			foreach($data['style'] AS $type=>$stdata)
			{
				if (!isset($this->_style[strtolower($type)]) || !stristr($this->_style[strtolower($type)],$stdata)) {
					/**
					 * 2012-05-07 wengebin edit
					 * 增加输出 css 前的处理,将 css 字符串分割并去重复,再重新拼凑即可!
					 */
					$this->addStyleDeclaration($stdata, $type);
					$splice_styles = explode('}', $this->_style[strtolower($type)]);
					$splice_styles = array_unique($splice_styles);
					$this->_style[strtolower($type)] = implode('}', $splice_styles);
 				}
			}
		}
	...
	public function getBuffer($type = null, $name = null, $attribs = array())
	{
		 ...
		 * 
		 * 加载一个渲染器,类型是 module,位置是:
		 * libraries/joomla/document/html/renderer/module.php
		 * 
		 * 对存取缓存的 hash 值进行了重构，为了防止css重复添加！
		 */
		...
		$clone_renderer = clone $renderer;
		if ($this->_caching == true && $type == 'modules') {
			$hash = md5(serialize(array($name, $attribs, $result, $clone_renderer)));
			$cache = JFactory::getCache('com_modules','');
			$cbuffer = $cache->get('cbuffer_'.$type.$Itemid.$catid.$pid);
//echo 'HASH:'.$hash.' - '.$name.';<br/>';
			if (isset($cbuffer[$hash])) {
				...
				$cache->store($cbuffer, 'cbuffer_'.$type.$Itemid.$catid.$pid);
			}
