...
class JDocument extends JObject
{
	...
	/**
	 * 2012-05-16 wengebin add
	 * 
	 * Array of linked scripts and cannot remove
	 *
	 * @var		array
	 */
	public $_nrm_scripts = array();
	 ...
	 * @since    11.1
	 */
	public function removeScript($script_url) {
		if (in_array($script_url,$this->_nrm_scripts)) {
			return true;
		}
		$temp_scripts = array();
		if (count($this->_scripts)) {
			foreach ($this->_scripts as $key=>$s) {
				if ($key != $script_url) {
					$temp_scripts[$key] = $s;
				}
			}
		}
		$this->_scripts = $temp_scripts;
	}
	
	/**
	 * 2011-05-16 wengebin Add
	 * 
	 * Set cannot be removed script
	 *
	 * @return
	 * @since    11.1
	 */
	public function notRemoveScript($script_url) {
		$this->_nrm_scripts[] = $script_url;
		return true;
	}
	...
	public function addStyleDeclaration($content, $type = 'text/css')
	{
		if (!isset($this->_style[strtolower($type)])) {
			...
			$this->_style[strtolower($type)] = $content.$this->_style[strtolower($type)];
		}
	}
