...
//var_dump($identities);echo '<br/>';
//if ($asset == 'com_service') {
//	var_dump(self::$assetRules[$asset]);echo '<br/>';
//}
...
//echo $results[$storeId].'<br/>';
	...
	/**
	 * 2011-12-7 wengebin edit
	 * 制定义权限检查，主要是自定义组件没有 lft 与 rgt，因此需要避免查找到所有自定义组件。
	 */
	public static function checkInCustom($userId, $action, $asset = null)
	{
		// Sanitise inputs.
		$userId = (int) $userId;
		...
		// Get the rules for the asset recursively to root if not already retrieved.
		if (empty(self::$assetRules[$asset])) {
			self::$assetRules[$asset] = self::getAssetRulesByComName($asset, true);
		}
...
//}
	...
	/**
	 * 2011-12-6 wengebin edit
	 * 根据组ID验证 action 是否有访问权限。
	 */
	public static function checkGroupByGroupId($groupId, $action, $asset = null)
	{
		// Sanitize inputs.
		$groupId = (int) $groupId;
		$action = strtolower(preg_replace('#[\s\-]+#', '.', trim($action)));
...
//var_dump($groupPath);echo '<br/><br/>';
	...
	/**
	 * 2011-12-6 wengebin edit
	 * 根据组访问组件的 name 查找权限，但已经限定在了单个组件。
	 */
	public static function getAssetRulesByComName($asset, $recursive = false)
	{
		// Get the database connection object.
		$db = JFactory::getDbo();
		...
		if (is_numeric($asset)) {
			...
			$query->where(array('a.name = '.$db->quote($asset),'b.name = '.$db->quote($asset)));
		}
