 ...
 * Hr Model
 */
class HrModelHr extends JModelAdmin
{
	/**
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authoriseInCustom('core.edit', 'com_hr.message.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	/**
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Hr', $prefix = 'HrTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hr.hr', 'hr', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	{
		return 'administrator/components/com_hr/models/forms/hr.js';
	}
	/**
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_hr.edit.hr.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
			...
			if (!$this->getState('filter.category_id')) {
				...
				$data->set('catid', JRequest::getInt('catid', $app->getUserState('com_hr.hrs.filter.category_id')));
			}
		}
		return $data;
	}
	...
	function published(&$pks, $value = 1)
	{
		...
		foreach ($pks as $i => $pk)
		{
			if ($table->load($pk)) {
				...
				$allow	= $user->authoriseInCustom('core.edit.state', 'com_hr');
