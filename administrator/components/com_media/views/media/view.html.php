...
class MediaViewMedia extends JView
{
	...
	protected function addToolbar()
	{
		...
		$layout = JRequest::getVar('layout');
		if ($layout == 'watermark') {
			JToolBarHelper::title(JText::_('COM_WATERMARK'), 'mediamanager.png');
		} else {
		...
		if ($layout == 'watermark')
		{
			...
			$dhtml = "<a href=\"#\" onclick=\"Joomla.submitbutton('file.saveWatermark')\" class=\"toolbar\">
						<span class=\"icon-32-apply\" title=\"$title\"></span>
						$title</a>";
			$bar->appendButton('Custom', $dhtml, 'save');
			
			$title = JText::_('JTOOLBAR_CANCEL');
			$dhtml = "<a href=\"#\" onclick=\"Joomla.submitbutton('file.cancel')\" class=\"toolbar\">
						<span class=\"icon-32-cancel\" title=\"$title\"></span>
						$title</a>";
		...
		// Add a delete button
		if ($user->authorise('core.delete','com_media') && $layout != 'watermark')
		{
			$title = JText::_('COM_MEDIA_WATERMARK');
			$dhtml = "<a href=\"index.php?option=com_media&layout=watermark\" class=\"toolbar\">
						<span class=\"icon-32-archive\" title=\"$title\"></span>
						$title</a>";
			$bar->appendButton('Custom', $dhtml, 'archive');
