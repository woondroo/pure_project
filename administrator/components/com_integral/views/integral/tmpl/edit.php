...
?>
<form action='<?php echo JRoute::_('index.php?option=com_integral&layout=edit&id='.(int) $this->item->id); ?>' method='post' name='adminForm' id='integral-form' class='form-validate'>
<?php
/**
...
?>
	<div class='width-60 fltlft'>
		<fieldset class='adminform'>
			<legend><?php echo JText::_( 'COM_INTEGRAL_INTEGRAL_DETAILS' ); ?></legend>
...
foreach($this->form->getFieldset('details') as $field):
	if ($field->name == 'jform[image]') {
		...
		$upload_topro_pid = 1; // 设置是否开启图片描述，如果不开启这赋值为0即可，如果传递了id，则需要数据库有 imgdesc 字段，默认不开启！
		$upload_topro_table = 'product'; // 设置图片描述所在的数据库表，如果为 product 则表示修改 #__product 表中的 imgdesc 字段，默认为 product！
		include(JPATH_SITE.'/administrator/templates/system/html/com_media/upload/upload.php');
	} else {
?>
...
?>
	...
	<div class='width-40 fltrt'>
		<?php echo JHtml::_('sliders.start', 'integral-slider'); ?>
...
<?php endforeach; ?>
	...
	<div>
		<input type='hidden' name='task' value='integral.edit' />
