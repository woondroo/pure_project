 ...
 * Integral Model
 */
class IntegralModelIntegral extends JModelAdmin
{
	/**
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authoriseInCustom('core.edit', 'com_integral.message.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	/**
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Integral', $prefix = 'IntegralTable', $config = array()) 
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_integral.integral', 'integral', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	{
		return 'administrator/components/com_integral/models/forms/integral.js';
	}
	/**
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_integral.edit.integral.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
	...
	function published(&$pks, $value = 1)
	{
		...
		foreach ($pks as $i => $pk)
		{
			if ($table->load($pk)) {
				...
				$allow	= $user->authoriseInCustom('core.edit.state', 'com_integral');
