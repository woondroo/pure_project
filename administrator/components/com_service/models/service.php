...
class ServiceModelService extends JModelAdmin
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authoriseInCustom('core.edit', 'com_service.message.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	/**
	...
	{
		return 'administrator/components/com_service/models/forms/service.js';
	}
	/**
	...
	function published(&$pks, $value = 1)
	{
		...
		foreach ($pks as $i => $pk)
		{
			if ($table->load($pk)) {
				...
				$allow	= $user->authoriseInCustom('core.edit.state', 'com_service');
