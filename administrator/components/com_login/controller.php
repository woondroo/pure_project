...
class LoginController extends JController
{
	...
	public function login()
	{
		...
		$word = JRequest::getVar('code', false, '', '');
		$get_confirm = MulanImageUtil::onCaptcha_confirm($word, $return);
		if (!$get_confirm) {
			JError::raiseWarning('','验证码错误' );
			LoginController::display();return;
	    }
		
		$model = $this->getModel('login');
		$credentials = $model->getState('credentials');
		$get_confirm = $model->getState('return');

		$result = $app->login($credentials, array('action' => 'core.login.admin'));

		if (!JError::isError($result)) {
			$app->redirect($get_confirm);
		}

		parent::display();
	}
	/**
	 * 2012-05-16 wengebin edit
	 * 
	 * 修改验证码获得方式，直接调用 mulanimage 中的验证码获
	 */
	public function displaycaptcha(){
		return  MulanImageUtil::onCaptcha_Display();
	}
	/**
