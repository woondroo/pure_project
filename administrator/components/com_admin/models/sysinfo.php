...
class AdminModelSysInfo extends JModel
{
	...
	function &getPhpSettings()
	{
		...
		{
			/**
			 * 2012-06-27 wengebin edit
			 * 
			 * 增加检测项
			 */
			$this->php_settings = array();
			$this->php_settings['php_version']			= version_compare(PHP_VERSION, '5.2.4', '>=');
			$this->php_settings['mysql_connect']		= (function_exists('mysql_connect') || function_exists('mysqli_connect'));
			if (extension_loaded('mbstring')) {
				$this->php_settings['mbstring.language'] = (strtolower(ini_get('mbstring.language')) == 'neutral');
				$this->php_settings['mbstring.func_overload'] = (ini_get('mbstring.func_overload') == 0);
			}
			$this->php_settings['ini_parser']			= $this->getIniParserAvailability();
			$this->php_settings['json_support']			= function_exists('json_encode') && function_exists('json_decode');
			$this->php_settings['configuration']		= (is_writable(JPATH_ROOT.'/configuration.php')) || (!file_exists(JPATH_ROOT.'/configuration.php') && is_writable(JPATH_ROOT));
