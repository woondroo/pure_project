...
class DownloadModelDownload extends JModelAdmin
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authoriseInCustom('core.edit', 'com_download.message.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	/**
	...
	{
		// Get the form.
		$form = $this->loadForm('com_download.download', 'download', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	{
		return 'administrator/components/com_download/models/forms/download.js';
	}
	/**
	...
	protected function loadFormData() 
	{
		...
		if (empty($data)) 
		{
			...
			if (!$this->getState('filter.category_id')) {
				...
				$data->set('catid', JRequest::getInt('catid', $app->getUserState('com_download.downloads.filter.category_id')));
			}
		}
		return $data;
	}
	...
	function published(&$pks, $value = 1)
	{
		...
		foreach ($pks as $i => $pk)
		{
			if ($table->load($pk)) {
				...
				$allow	= $user->authoriseInCustom('core.edit.state', 'com_download');
