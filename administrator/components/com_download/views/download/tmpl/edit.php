...
?>
<form action='<?php echo JRoute::_('index.php?option=com_download&layout=edit&id='.(int) $this->item->id); ?>' method='post' name='adminForm' id='download-form' class='form-validate'>
<?php
/**
...
?>
	<div class='width-60 fltlft'>
		<fieldset class='adminform'>
			<legend><?php echo JText::_( 'COM_DOWNLOAD_DOWNLOAD_DETAILS' ); ?></legend>
...
foreach($this->form->getFieldset('details') as $field):
	if ($field->name == 'jform[image]') {
		...
		$upload_others_type = array();
		include(JPATH_SITE.'/administrator/templates/system/html/com_media/upload/upload.php');
	} else if ($field->name == 'jform[file]') {
		$set_swfupload_title = $this->form->getLabel('file');
		$file_upload_folder = $upload_uri;
		$file_input_id = 'jform_file';
		$file_input_name = 'jform[file]';
		$file_value = $this->item->file;
		$file_view_id = '';
		$file_norename = 0;
		$file_upload_types = array('document');
		include(JPATH_SITE.'/administrator/templates/system/html/com_media/upload/upload.php');
	} else {
?>
		<li><?php
...
endforeach; ?>
	...
	<div class='width-40 fltrt'>
		<?php echo JHtml::_('sliders.start', 'download-slider'); ?>
