 ...
 */
class SitelinkxControllerSitelinkxlist extends SitelinkxController{
	...
	public function edit(){
		...
		$view =& $this->getView('sitelinkx', 'html');
	...
	public function save(){
		$model = $this->getModel('sitelinkxlist'); 
	...
	public function publish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__sitelinkx SET published = 1 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
		$this->setMessage( JText::sprintf('Fields published', $n ) );
		$this->setRedirect( 'index.php?option=com_sitelinkx&controller=sitelinkxlist' );
	}
	...
	public function unpublish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__sitelinkx SET published = 0 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
	...
	function cancel() {
		$model = $this->getModel( 'sitelinkx' );
		$model->checkin();
		$this->setRedirect( 'index.php?option=com_sitelinkx&view=sitelinkxlist' );
	}
	*/
