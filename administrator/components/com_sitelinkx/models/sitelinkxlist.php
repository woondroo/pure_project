 ...
 */
class SitelinkxModelSitelinkxlist extends JModel{
	/**
	 * Sitelinkxlist data array
	 *
	 * @var array
	 */
	private $_data;
	...
	*/
	...
	private function _buildQuery(){
		...
		$query = 'SELECT t1.* FROM `#__sitelinkx` t1 ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'id,wort,ersatz,schlagwort,fenster,published,begpub,endpub,anzahl,suchm'); // array('id', 'ordering', 'published'); 

		// retrive ordering info
		$filter_order = $app->getUserStateFromRequest('com_sitelinkxfilter_order', 'filter_order', $default_order_field);
		$filter_order_Dir = strtoupper($app->getUserStateFromRequest('com_sitelinkxfilter_order_Dir', 'filter_order_Dir', 'ASC'));
	...
	private function _buildQueryWhere() {
		...
		$search = $app->getUserStateFromRequest('com_sitelinkxsearch', 'search', '');
	...
	public function getTotal(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*) FROM `#__sitelinkx` t1 ' . $this->_buildQueryWhere() );
	...
	public function &getGenericFieldName($fieldName){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT id AS value `$fieldName` AS text FROM `#__sitelinkx` ORDER BY `$fieldName`');
		$options = array();
