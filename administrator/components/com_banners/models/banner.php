...
class BannersModelBanner extends JModelAdmin
{
	...
	/**
	 * 2011-12-23 wengebin 添加！
	 * 
	 * 重写save方法，会在保存数据之前先对需要展示banner的页面进行关联！
	 */
	public function save($data){
		$result = parent::save($data);
		if ($data['coms'] && $data['id']) {
			$this->saveComs($data['coms'],$data['id']);
		} else if ($data['coms'] && $result) {
			$this->saveComs($data['coms'],$result);
		}
		return $result;
	}
	 ...
	 * 2011-12-23 wengebin 添加！
	 * 
	 * 保存关联数据，将banner与展示页关联起来！
	 */
	public function saveComs($coms,$id) {
		jimport('mulan.mldb');
		$sql = '';
		if ($id) {
			$sql .= 'delete from #__banners_coms where banner='.MulanDBUtil::dbQuote($id).';';
			MulanDBUtil::executeSql($sql);
			$sql = '';
		}
		$coms_array = explode(',',$coms);
		if (count($coms_array)) {
			$sql .= 'insert into #__banners_coms(`banner`,`com`) values ';
			foreach ($coms_array as $key=>$c) {
				$sql .= $key == 0 ? '('.MulanDBUtil::dbQuote($id).','.MulanDBUtil::dbQuote($c).')' : ',('.MulanDBUtil::dbQuote($id).','.MulanDBUtil::dbQuote($c).')';
			}
		}
		MulanDBUtil::executeSql($sql);
	}
	...
	protected function loadFormData()
	{
		...
		if (empty($data)) {
			...
			jimport('mulan.mldb');
			$coms = MulanDBUtil::getObjectlistBySql('select * from #__banners_coms where banner='.MulanDBUtil::dbQuote($data->id));
			if (count($coms)) {
				$coms_str = '';
				foreach ($coms as $key=>$c) {
					$coms_str .= $key == 0 ? $c->com : ','.$c->com;
				}
				$data->coms = $coms_str;
			}
