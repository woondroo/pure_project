...
<form action="<?php echo JRoute::_('index.php?option=com_users&layout=edit&id='.(int) $this->item->id); ?>" method="post" name="adminForm" id="user-form" class="form-validate">
	<div class="width-60 fltlft">
		<fieldset class="adminform">
			...
			<?php
				foreach($this->form->getFieldset('user_details') as $field) :
					if ($field->name == 'jform[integral]') {
						if (!$site_model) continue;
			?>
			<li>
			...
			<?php
					} else if ($field->name == 'jform[integral_reason]') {
					} else if ($field->name == 'jform[addintegral]') {
						if (!$site_model) continue;
			?>
			<li>
				<?php echo $field->label; ?>
