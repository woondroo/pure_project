...
class UsersModelUser extends JModelAdmin
{
	...
	*/
	...
	public function save($data)
	{
		...
		$reason = $data['integral_reason'];
		
		jimport('mulan.mldb');
		$data['addintegral'] = intval($data['addintegral']);
		$integral_history = MulanDBUtil::getObjectBySql('select integral_history from #__users where id='.MulanDBUtil::dbQuote($data['id']))->integral_history;
		$data['integral'] = intval($data['integral']) + $data['addintegral'];
		$data['integral_history'] = $integral_history + $data['addintegral'];
		if ($data['addintegral'] > 0) {
			MulanDBUtil::executeSql('insert into #__integral_history(`pid`,`uid`,`reason`,`use`,`get`,`last`,`ordertime`,`receivetime`,`completetime`,`state`,`way`) values (\'0\',\''.$data['id'].'\','.MulanDBUtil::dbQuote($reason).',\'0\','.MulanDBUtil::dbQuote($data['addintegral']).','.$data['integral'].',\'0000-00-00 00:00:00\',\'0000-00-00 00:00:00\',\''.date('Y-m-d H:i:s').'\',\'2\',\'0\')');
		} else if ($data['addintegral'] < 0) {
			MulanDBUtil::executeSql('insert into #__integral_history(`pid`,`uid`,`reason`,`use`,`get`,`last`,`ordertime`,`receivetime`,`completetime`,`state`,`way`) values (\'0\',\''.$data['id'].'\','.MulanDBUtil::dbQuote($reason).','.MulanDBUtil::dbQuote(abs($data['addintegral'])).',\'0\','.$data['integral'].',\'0000-00-00 00:00:00\',\'0000-00-00 00:00:00\',\''.date('Y-m-d H:i:s').'\',\'2\',\'1\')');
		}
		unset($data['addintegral']);
		unset($data['integral_reason']);
