...
<form action="<?php echo JRoute::_('index.php?option=com_weblinks&view=weblinks'); ?>" method="post" name="adminForm" id="adminForm">
	<fieldset id="filter-bar">
		...
		<a id='save-list-ordering' class='custome-button-red fltrt' href='index.php?option=<?php echo JRequest::getVar('option') ?>&task=<?php echo substr(JRequest::getVar('option'),4)?>.saveOrderByOrder&orderfield=<?php echo $this->state->get('list.ordering'); ?>&orderby=<?php echo $this->state->get('list.direction'); ?>&filter_category_id=<?php echo JRequest::getVar('filter_category_id')?JRequest::getVar('filter_category_id'):0?>'>
			保存当前排序
		</a>
