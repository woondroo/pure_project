...
abstract class JToolBarHelper
{
	 ...
	 * 
	 * Writes a common 'Export' button for a list of records.
	 *
	 * @param	string	$msg	Postscript for the 'are you sure' message.
	 * @param	string	$task	An override for the task.
	 * @param	string	$alt	An override for the alt text.
	 * @since	1.0
	 */
	public static function exportList($msg = '', $task = 'expexcel', $alt = 'JTOOLBAR_EXPORTEXCEL')
	{
		$bar = JToolBar::getInstance('toolbar');
		// Add a export button.
		if ($msg) {
			$bar->appendButton('Confirm', $msg, 'expexcel', $alt, $task, true);
		} else {
			$bar->appendButton('Standard', 'expexcel', $alt, $task, true);
		}
	}
}
