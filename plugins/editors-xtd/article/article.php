...
class plgButtonArticle extends JPlugin
{
	...
	function onDisplay($name)
	{
		 ...
		 * 修改文章插入编辑器代码，为了兼容多个编辑器，必须给不同编辑器的调用函数重新命名！
		 */
		$alias_name = str_replace(array('jform','[params]','[',']'),'',$name);
		$js = "
		function jSelectArticle_".$alias_name."(id, title, catid, object) {
			var tag = '<a href='+'\"index.php?option=com_content&amp;view=article&amp;catid='+catid+'&amp;id='+id+'\">'+title+'</a>';
			jInsertEditorText_".$alias_name."(tag, '".$name."');
			SqueezeBox.close();
		}";
		 ...
		 */
		$link = 'index.php?option=com_content&amp;view=articles&amp;layout=modal&amp;tmpl=component&amp;formname='.$alias_name;
